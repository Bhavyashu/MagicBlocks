...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating landowner Identities"

    cryptogen generate --config=./organizations/cryptogen/crypto-config-landowner.yaml --output="organizations"
    res=$?
    ...
    infoln "Creating landbuyer Identities"

    cryptogen generate --config=./organizations/cryptogen/crypto-config-landbuyer.yaml --output="organizations"
    res=$?
    ...
    infoln "Creating blockgaurdians Identities"

    cryptogen generate --config=./organizations/cryptogen/crypto-config-blockgaurdians.yaml --output="organizations"
    res=$?
    ...
    infoln "Creating insurance Identities"
  ...
  while :
    do
      if [ ! -f "organizations/fabric-ca/landowner/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    if [ "$OS_ARCH" == "darwin-amd64" ]; then
      ...
      rm -rf organizations/fabric-ca/landowner
      rm -rf organizations/fabric-ca/landbuyer
      rm -rf organizations/fabric-ca/blockgaurdians
      rm -rf organizations/fabric-ca/insurance
      rm -rf organizations/fabric-ca/government
      ...
      ## remove fabric ca artifacts
      docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/landowner'
      docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/landbuyer'
      docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/blockgaurdians'
      docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/insurance'
      docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/government'
